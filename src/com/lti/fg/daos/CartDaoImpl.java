...
public class CartDaoImpl implements CartDao{
    ...
    private EntityManager manager;
		...
		public int getQuantityFromHistory(int userId, int productId) throws CartException {
			 Query query = manager.createQuery("select quantity from OrderHistory where userId = :userId and productId = :productId");
		     query.setParameter("userId", userId);
		     query.setParameter("productId", productId);
			 int prodQuant = (int)query.getSingleResult();
			 return prodQuant;
		...
		public boolean deleteRecordById(int userId) throws CartException {
			...
			Query query = manager.createQuery("from Cart where userId = :userId");
			query.setParameter("userId", userId);
		...
		public boolean deleteRecordByProduct(int productId) throws CartException {
			Query query = manager.createQuery("delete from Cart where productId = "+productId);
